...
def load_data(config):
    ...
    if exp_setting == 'intra-subject':
        ...
        leave_one_subjects = sorted(set(all_subjects) - set(subjects))
        ...
        use_multiview = config.get('train', {}).get('lejepa', False)
        train_dataset = EEGDataset(leave_one_subjects_config,mode='train', multiview=use_multiview, left_out_subjects=subjects)
        print(f'init train_dataset success (multiview={use_multiview}, left_out={subjects})')
        test_loader = DataLoader(test_dataset, batch_size=config['data']['test_batch_size'], shuffle=False, drop_last=False,num_workers=25)#, pin_memory=True)
...
class EEGDataset(Dataset):
    def __init__(self, config, mode, multiview=False, left_out_subjects=None):
        self.config= config
        self.multiview = multiview
        self.left_out_subjects = left_out_subjects or []
        self.sampling_rate = config['data'].get('sampling_rate', 250)
        ...
        if self.multiview:
            self.match_label = np.ones(self.trial_subject, dtype=int)
        else:
    ...
    def __getitem__(self, index):
        if self.multiview:
            return self._getitem_multiview(index)
    ...
    def _getitem_multiview(self, index):
        """Return all subjects' EEG for stimulus `index` as (V, C, T)."""
        n_subjects = len(self.subjects)
        
        # Verify label consistency across subjects
        ref_label = self.loaded_data[0]['label'][index]
        for s in range(1, n_subjects):
            assert self.loaded_data[s]['label'][index] == ref_label, \
                f"Stimulus {index}: label mismatch between subject 0 and {s}"
        
        # Stack EEG from all subjects: (V, C, T)
        eeg = torch.stack([
            self.loaded_data[s]['eeg'][index].float()[:, self.timesteps[0]:self.timesteps[1]]
            for s in range(n_subjects)
        ])
        
        label = ref_label
        img_path = self.loaded_data[0]['img'][index]
        ...
        text_raw = self.loaded_data[0]['text'][index]
        text = f"This is a {text_raw}."
        text_features = self.text_features[text_raw]
        ...
        sample = {
            ...
            'text': text,
            'text_features': text_features,
            'session': self.loaded_data[0]['session'][index],  # from reference subject
            'subject': n_subjects,    # number of views; left-out subject(s): self.left_out_subjects
            'eeg_mean': eeg.mean(dim=0),  # mean across subjects: (C, T)
        }
        return sample
    
    def __len__(self):
        if self.multiview:
            return self.trial_subject
