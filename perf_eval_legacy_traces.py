"""
Legacy-like performance evaluation with lightweight per-step throughput traces.

This file stays close to perf_eval_legacy.py on purpose:
  ...
  - same dataset implementations and DataLoader settings

The only behavioral addition is optional per-step trace recording so the JSON
report can drive step-throughput figures directly.
"""
...
def benchmark_dataloader(
    ...
    global_step = 0
    trace_stride = max(1, int(trace_stride))
    ...
    for epoch in range(num_epochs):
        ...
        for step_in_epoch, batch in enumerate(loader, start=1):
            ...
            if record_traces and ((global_step % trace_stride) == 0):
                step_traces.append({
                    "label": label,
        ...
        epoch_times.append(t_epoch_end - epoch_start)
...
def run_experiment(args):
    ...
    print(f"  record_traces    : {not args.disable_traces}")
        ...
        for nw in args.num_workers:
            ...
            for name, ds in variants:
                ...
                result, traces = benchmark_dataloader(
                    ...
                    label=name,
                    record_traces=not args.disable_traces,
                    trace_stride=args.trace_stride,
                )
                ...
                if not args.disable_traces:
                    print(f"    Trace pts  : {result['trace_length']}")
                print()
...
def main():
    parser = argparse.ArgumentParser(
        description="Legacy-like benchmark with optional per-step throughput traces",
    ...
    parser.add_argument("--disable_traces", action="store_true", help="Disable per-step trace recording")
    parser.add_argument(
        "--trace_stride",
        type=int,
        default=1,
        help="Record every Nth measured step to reduce trace size and overhead",
    )
